...
,状态价值不一定大,参考贝尔曼方程计算公式.如果我们使用另一种价值函数的话,就是q值,代表状态S下采取特定动作A的价值,
那么我们就可以说,终点前一个状态,采取动作A可以直接到终点,这个q(S,A)一定是一个大值,
这边也提示了大家使用q值一般比使用状态价值V(s)更好,因为考虑了动作,这两个价值函数下面就说到)
